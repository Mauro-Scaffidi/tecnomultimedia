...
class Juego {
  ...
  Juego() {
    ...
    /*Se asigna un valor a cada espacio del arreglo pantallas[n]. Ese valor es una instancia de 
     la clase Pantalla y de esa forma se dibuja todo lo que tiene que tener la pantalla salvo 
     el texto y los botones. Para agregar esos elementos a las pantallas del programa se llama a 
     la funcion "agregarTexto" de la clase Pantalla y dentro de sus parametros se escribe las lineas 
     de lo que se quiere escribir.
     */
    ...
    //Para evitar NullPointerException agregando el boton una vez que estan creadas las dos pantallas
    pantallas[15].agregar1Opcion(this, "", null);


    //Se agregan los botones a las pantallas
    /*Se llama a las funciones "agregae1Opcion" y "agregar2Opciones" de la clase 
     Pantalla para agregar los botones a las pantallas creadas anteriormente en 
     el arreglo*/
  ...
  /*En las funciones draw y mouseClicked solo se escriben las funciones draw y mouseClicked de pantallaActual.
   Esto debido a que la variable pantallaActual es de tipo Pantalla y por esto sabe a que clase pedir las funciones correspondientes*/
  void draw() {
    pantallaActual.draw();
  }
